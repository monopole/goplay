...
import (
	...
	// This means that in this program, where we want to use both v1 and v2,
	// simulataneously, we must alias v1 explicitly:
	applev1 "github.com/monopole/goplay/apple"
	
	// Go knows that when a module path ends in major version indicator like v2, v3, etc.
	// that it should implicitly strip that indicator, so that the bare package name `apple.`
	// can be used in the code, rather that requiring the code to specify an ambiguous `v2.`,
	// or an alias like that used above for v1.
